...
export default function ExperimentLab() {
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    <div className="grid lg:grid-cols-[1.1fr_.9fr] gap-5"><Card><form onSubmit={run} className="space-y-4"><input value={name} onChange={e => setName(e.target.value)} className="w-full bg-elevated border border-border rounded-lg px-3 py-2.5 text-sm text-ink outline-none focus:border-accent" placeholder="Experiment name" />
      <div className="rounded-xl border border-accent/20 bg-accent/5 p-3 space-y-2"><p className="text-xs font-semibold text-accent-soft">Generate a live candidate with Gemini</p><textarea rows={3} value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Write the prompt you want to test..." className="w-full bg-elevated border border-border rounded-lg px-3 py-2.5 text-sm text-ink outline-none focus:border-accent resize-y" /><Button type="button" variant="secondary" size="sm" onClick={generateCandidate} disabled={generating || !prompt.trim()}>{generating ? 'Calling Gemini...' : 'Generate candidate'}</Button></div>
