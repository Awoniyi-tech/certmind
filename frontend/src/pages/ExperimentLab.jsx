...
export default function ExperimentLab() {
  ...
  async function generateCandidate() {
    ...
    try {
      const result = await ragAPI.runPrompt({ prompt })
      setCandidates(items => [...items, { name: `Gemini candidate ${items.length + 1}`, response: result.text }])
    } catch (err) { setError(err.response?.data?.detail || 'Model execution failed. Check GOOGLE_API_KEY.') }
    finally { setGenerating(false) }
  }
  async function compareModels() {
    if (!prompt.trim()) return
    setGenerating(true); setError('')
    try { setModelResult(await ragAPI.compareModels({ prompt, models: models.split(',').map(item => item.trim()).filter(Boolean) })) }
    catch (err) { setError(err.response?.data?.detail || 'Model comparison failed.') }
    finally { setGenerating(false) }
  }
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    <div className="grid lg:grid-cols-[1.1fr_.9fr] gap-5"><Card><form onSubmit={run} className="space-y-4"><input value={name} onChange={e => setName(e.target.value)} className="w-full bg-elevated border border-border rounded-lg px-3 py-2.5 text-sm text-ink outline-none focus:border-accent" placeholder="Experiment name" />
      <div className="rounded-xl border border-accent/20 bg-accent/5 p-3 space-y-2"><p className="text-xs font-semibold text-accent-soft">Run a live model comparison</p><textarea rows={3} value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Write the prompt you want to test..." className="w-full bg-elevated border border-border rounded-lg px-3 py-2.5 text-sm text-ink outline-none focus:border-accent resize-y" /><div className="flex gap-2"><input value={models} onChange={e => setModels(e.target.value)} placeholder="Models, comma separated" className="flex-1 bg-elevated border border-border rounded-lg px-3 py-2 text-xs text-ink outline-none" /><Button type="button" variant="secondary" size="sm" onClick={compareModels} disabled={generating || !prompt.trim()}>{generating ? 'Running...' : 'Compare models'}</Button></div><Button type="button" size="sm" onClick={generateCandidate} disabled={generating || !prompt.trim()}>Generate one candidate</Button>{modelResult && <div className="pt-2 space-y-2">{modelResult.results.map(item => <div key={item.model} className="flex items-center justify-between rounded-lg bg-elevated/60 px-3 py-2 text-xs"><span className="text-ink">{item.model}</span><span className={item.status === 'completed' ? 'text-success' : 'text-danger'}>{item.status === 'completed' ? `${item.latency_ms}ms · $${item.estimated_cost_usd}` : item.status}</span></div>)}</div>}</div>
